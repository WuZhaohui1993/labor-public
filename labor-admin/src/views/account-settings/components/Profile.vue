...
import { syncUserSnapshot } from "@/utils/auth";
...
const handleSubmitImage = async () => {
  if (!cropperBlob.value) {
    message("请先调整头像", { type: "warning" });
    return;
  }
  try {
    const result = await updateMineAvatar(
      new File([cropperBlob.value], "avatar.png", { type: "image/png" })
    );
    if (result.code !== "OK") throw new Error(result.message);
    Object.assign(userInfos, result.data);
    syncUserSnapshot(result.data);
...
const onSubmit = async (formEl?: FormInstance) => {
  ...
  await formEl.validate(async valid => {
    if (!valid) return;
    try {
      const result = await updateMineProfile({
      ...
      message(error instanceof Error ? error.message : "个人信息更新失败", {
        type: "error"
      });
    }
  });
